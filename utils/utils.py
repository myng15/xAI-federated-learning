...
def get_loaders(type_, aggregator_, data_dir, batch_size, is_validation):
    """
    ...
    for task_id, task_dir in enumerate(tqdm(os.listdir(data_dir))):
        ...
        train_iterator = \
            get_loader(
                type_=type_,
                aggregator_=aggregator_, #added argument to accommodate embedding databases as input in non-parametric mode (parametric mode such as FedAvg requires raw image datasets as input)
...
def get_client(
    ...
    """
    if client_type == "fedknnrelative":
        return KNNRelativeSimilaritiesClient(
            learner=learner,
